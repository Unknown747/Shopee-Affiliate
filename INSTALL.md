...
5. **Catat kredensial admin** yang ditampilkan di akhir output (juga tersimpan di `.env`):

   ```
   username : admin
   password : <auto-generated 20 char>
   ```
...
| 8/8  | `mkdir -p logs/`, `pm2 start ecosystem.config.cjs`, `pm2 save`                    | ✅ reload kalau sudah running |
...
> Atau cukup jalankan `./install.sh` lagi — script-nya idempotent dan akan update build + reload PM2.
