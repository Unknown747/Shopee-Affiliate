...
Panduan lengkap untuk men-deploy platform Shopee Affiliate ini ke VPS sendiri (Ubuntu 22.04 / 24.04).
...
7. [Setup Database (Migrasi & Seed)](#7-setup-database-migrasi--seed)
...
(Opsional) Seed data contoh produk — jika ada script seed di `scripts/`:

```bash
pnpm --filter scripts run seed   # cek dulu nama script di scripts/package.json
```
...
Buat file konfigurasi PM2 di root project:

```bash
nano ~/shopee-affiliate/ecosystem.config.cjs
```
...
module.exports = {
  apps: [
    {
      name: "shopee-api",
      cwd: "/home/deploy/shopee-affiliate/artifacts/api-server",
      script: "node",
      args: "--enable-source-maps ./dist/index.mjs",
      env_file: "/home/deploy/shopee-affiliate/.env",
      env: {
        NODE_ENV: "production",
        PORT: "8080",
      },
      ...
      out_file: "/home/deploy/logs/shopee-api.out.log",
      error_file: "/home/deploy/logs/shopee-api.err.log",
      time: true,
    },
    {
      name: "shopee-web",
      cwd: "/home/deploy/shopee-affiliate/artifacts/shopee-affiliate",
      script: "pnpm",
      args: "run serve",
      env_file: "/home/deploy/shopee-affiliate/.env",
      env: {
      ...
      out_file: "/home/deploy/logs/shopee-web.out.log",
      error_file: "/home/deploy/logs/shopee-web.err.log",
      time: true,
    },
  ],
};
...
```bash
mkdir -p ~/logs
pm2 start ~/shopee-affiliate/ecosystem.config.cjs
pm2 status
pm2 logs --lines 30
```
...
```
/home/deploy/
├── shopee-affiliate/           ← project root
│   ├── .env                    ← rahasia, chmod 600
│   ├── ecosystem.config.cjs    ← konfigurasi PM2
│   ├── artifacts/
│   │   ├── api-server/         ← Express backend
│   │   └── shopee-affiliate/   ← React frontend
│   ├── lib/db/                 ← Drizzle schema
│   └── pnpm-workspace.yaml
├── logs/                       ← log PM2
└── backups/                    ← dump PostgreSQL
```
